...
export default function ReportPDF() {
  const { actData, pep, material, weld, reflectors } = useStore();
  ...
  return (
        ...
        <Page size="A4" style={styles.page}>
          ...
          {reflectors.map((r, i) => (
            <View key={r.id} style={styles.row}>
              ...
              <Text style={styles.cell}>{r.desc || `X=${r.x} Y=${r.y}`}</Text>
    ...
    } fileName={`UT_Act_${dateStr.replace(/\./g,'-')}.pdf`}>
      {({ loading }) => (
        <button disabled={loading} className="w-full mt-4 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 disabled:opacity-50 font-medium">
